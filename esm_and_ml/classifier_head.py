...
class ESMWithClassifier(nn.Module):
    def __init__(self, device, input_dim, output_dim=1):
        super(ESMWithClassifier, self).__init__()
        self.device = device
        # Load the pre-trained ESM model
        self.esm = AutoModelForMaskedLM.from_pretrained('models').to(device)
        # Instantiate the MLP
        self.decoder = MLP(input_dim=input_dim, output_dim=output_dim)
    ...
    def forward(self, sequences, attention_mask=None):
        ...
        mlp_output = self.decoder(pooled_hidden_states)
        return mlp_output 
